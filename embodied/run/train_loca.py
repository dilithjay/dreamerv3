...
def train_loca(
    ...
    make_logger,
    args):
  """Multi-phase LoCA training loop.

  Cloned from train_eval; adds phase transitions at cumulative-step boundaries
  derived from args.loca.phases_steps, broadcasting set_phase_and_eval to the
  train (is_eval=False) and eval (is_eval=True) drivers. The replay buffer is
  NOT reset across phases (matching r2dreamer): with a LoFo buffer, old-phase
  transitions are gradually displaced by density-based eviction.
  """
  ...
  # LoCA phase boundaries (cumulative env steps), mirroring r2dreamer's
  # OnlineTrainer: phases_steps=[a, b] -> boundaries=[a]; phase 1 runs until a,
  # phase 2 afterwards. With a single phase, no transitions occur.
  phases = list(args.loca.phases_steps)
  boundaries = []
  if args.loca.enabled and len(phases) > 1:
    cum = 0
    for s in phases[:-1]:
      cum += int(s)
  ...
  # Broadcast the initial phase to both drivers before the first rollout, so
  # envs spawn in phase-1 state.
  if boundaries:
    driver_train.call_env('set_phase_and_eval', 1, False)
    driver_eval.call_env('set_phase_and_eval', 1, True)

  def maybe_transition_phase():
    if not boundaries:
      return
    idx = current_phase[0] - 1
    if idx >= len(boundaries) or int(step) < boundaries[idx]:
      return
    cp.save()  # Checkpoint at the phase boundary.
    current_phase[0] += 1
    print(f'[loca] phase {current_phase[0] - 1} -> {current_phase[0]} '
          f'at step {int(step)}')
    driver_train.call_env('set_phase_and_eval', current_phase[0], False)
    driver_eval.call_env('set_phase_and_eval', current_phase[0], True)
